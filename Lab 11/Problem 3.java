...
class Edge {
    private final int fromVertex, toVertex, weight;
...
public class OrganizacijaTest {
    ...
    public static void main(String[] args) {
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            AdjacencyMatrixGraph<String> graph = new AdjacencyMatrixGraph<>(numVertices);
            HashMap<String,Integer> map = new HashMap<>();
            for (int i = 0; i < numEdges; i++) {
                String line = scanner.nextLine();
                String[] parts = line.split("\\s+");
                int source = Integer.parseInt(parts[src_index]);
                int destination = Integer.parseInt(parts[dest_index]);
                map.put(parts[1], source);
                map.put(parts[3], destination);
                graph.addEdge(source, destination, Integer.parseInt(parts[weight_index]));
            }
            String president = scanner.nextLine();
            System.out.println(
                    graph.prim(map.get(president))
