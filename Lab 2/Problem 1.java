...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        int sumLengths=0;
        String match=arr.get(0);
        for (int i=0; i<arr.getSize(); i++){
            sumLengths+=arr.get(i).length();
        }
        
        double averageLength = (double) sumLengths/arr.getSize();
        double minDiff=Math.abs(arr.get(0).length() - averageLength);
        
        for (int i=1; i<arr.getSize(); i++){
         double diff=Math.abs(arr.get(i).length() - averageLength);
         if (diff<minDiff){
             minDiff=diff;
             match=arr.get(i);
            }
