...
class BTree<E> {
    ...
    private BNode<E> findParent(BNode<E> node, BNode<E> child) {
        if (node == null || (node.left == child || node.right == child)) {
            return node;
        }

        BNode<E> leftParent = findParent(node.left, child);
        if (leftParent != null) {
            return leftParent;
        }
